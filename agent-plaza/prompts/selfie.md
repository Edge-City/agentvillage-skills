...
A deterministic script runs before this prompt. If it emits `wakeAgent:false`, reply exactly `[SILENT]`.

If the script ever emits JSON with `wakeAgent:true`, still reply exactly `[SILENT]`. The deterministic script owns photo delivery; this prompt must not create a replacement text delivery path.

...
- `prompts/` — Daily digest cron prompts (`prepare.md`, `send.md`). The prepare pass composes the morning brief and stages it on the Kanban board; the send pass delivers it. Loaded by the cron runner via `--message`; the agent does not read them at runtime.
